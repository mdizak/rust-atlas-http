...
impl HttpHeaders {
    ...
    pub fn from_vec(header_lines: &Vec<String>) -> Self {
        ...
        for line in header_lines {
            if let Some(cindex) = line.find(":") {
                ...
                pairs.entry(key.clone()).or_insert(Vec::new()).push(line[cindex+1..].trim().to_string());
    ...
    pub fn from_hash(headers: &HashMap<&str, &str>) -> Self {
        ...
        let pairs: HashMap<String, Vec<String>> = headers.iter().map(|(k, v)| {
            let value: Vec<String> = v.split(";").map(|e| e.trim().to_string()).collect();
            (k.to_string(), value)
        }).collect();

        let lower_map: HashMap<String, String> = headers.keys().map(|k| {
            (k.to_lowercase().to_string(), k.to_string())
        }).collect();
    ...
    pub fn get_lower_vec(&self, key: &str) -> Option<Vec<String>> {
        if let Some(hdr_key) = self.lower_map.get(key.to_lowercase().as_str()) {
    ...
    pub fn set(&mut self, key: &str, value: &str) {
        ...
        *self.lower_map.entry(key.to_lowercase().to_string()).or_insert(key.to_string()) = key.to_string();
    }
    ...
    pub fn add(&mut self, key: &str, value: &str) {
        self.pairs.entry(key.to_string()).or_insert(Vec::new()).push(value.to_string());
        *self.lower_map.entry(key.to_lowercase().to_string()).or_insert(key.to_string()) = key.to_string();
    }
