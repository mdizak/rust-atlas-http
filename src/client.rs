...
impl HttpClient {
    ...
    fn send_request(&self, req: &HttpRequest, dest_file: &String) -> Result<HttpResponse, Error> {
        ...
        // Connect
        let mut sock = self.connect(&uri, &port)?;

        // Check for non-ssl connection
        if uri.scheme() != "https" && self.proxy_type != ProxyType::None {
            panic!("Sorry, but crate currently only supports https:// requests.  Check back very shortly for updated version that supports HTTP and SOCKS5");
        }
        ...
        let mut reader = BufReader::with_capacity(2048, Box::new(stream));
        ...
        for l in header_lines.iter() {
            println!("{}", l);
        }
        let headers = HttpHeaders::from_vec(&header_lines);

        // Chunked transfer encoding
        if headers.has_lower("transfer-encoding") && headers.get_lower("transfer-encoding").unwrap().as_str() == "chunked" {
        ...
        // Get response
        let mut res = HttpResponse::new_full(&status, &headers, &body, &version, &reason);
    ...
    fn connect(&self, uri: &Url, port: &u16) -> Result<TcpStream, Error> {
    ...
    fn read_header(&self, reader: &mut BufReader<dyn Read>, req: &HttpRequest) -> Result<(String, u16, String, Vec<String>), Error> {
